...
oddFactorial 1 = 1
oddFactorial n = if odd n then n * oddFactorial(n-2) 
...
myGcd 0 b = b
myGcd a 0 = a
myGcd a b = if a>b then myGcd (a-b) b
            else myGcd a (b-a)
...
leftpad str n = if n <= length str then str
                else leftpad (" " ++ str) n
...
counthelper 1 = "1... "
counthelper n = show(n) ++ "... " ++ counthelper(n-1)
...
smallestDivisor n =if even n then 2
                   else modu n 3

modu :: Integer -> Integer -> Integer
modu n m = if n `mod` m == 0 then m
           else modu n (m+1)
