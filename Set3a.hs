...
firstHalf x = if (even (length x)) then take ((length x) `div` 2) x
...
capitalizeFirst (head:tail) =Data.Char.toUpper head : tail 
------------------------------------------------------------------------------
...
powers :: Int -> Int -> [Int]
powers k max = mulpower k max [1]
mulpower :: Int -> Int -> [Int] -> [Int]
mulpower k max inpower= if last inpower * k > max then inpower
                        else mulpower k max (inpower ++ [last inpower * k])
------------------------------------------------------------------------------
...
joinToLength n list = [x ++ y | x <- list, y <- list, n == (length x + length y)]
...
--   [] +|+ []            ==> []
(+|+) :: [a] -> [a] -> [a]
(+|+) lista listb = take 1 lista ++ take 1 listb
-- (+|+) lista listb = [take 1 lista, take 1 listb]
...
-- multiApp :: [a->a] -> [a->a]
multiApp f list init = f [x init| x <-list]
...
interpreter' [] _ _ = []
interpreter' (f: fs) i j = case f of
    "up" -> interpreter' fs i (j + 1)
    "down" -> interpreter' fs i (j - 1)
    "left" -> interpreter' fs (i-1) j
    "right" -> interpreter' fs (i+1) j
    "printX" -> show i : interpreter' fs i j 
    "printY" -> show j : interpreter' fs i j
