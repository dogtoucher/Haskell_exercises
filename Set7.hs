...
add x (Set [])=Set [x]
add x (Set xs)= case member x (Set xs) of False -> Set (sort (x:xs))
                                          True -> Set xs
...
data State = Start | Eggsdone | Flourdone | Sugardone | FSdone | Mixed |Error | Finished
  deriving (Eq,Show)

step :: State -> Event -> State
step Start AddEggs = Eggsdone
step Eggsdone AddFlour = Flourdone
step Eggsdone AddSugar = Sugardone
step Flourdone AddSugar = FSdone
step Sugardone AddFlour = FSdone 
step FSdone Mix = Mixed
...
instance (Ord a) => Semigroup (Set a) where 
  (Set a) <> (Set b) = Set (sort(union a b))
...
passwordAllowed a (MinimumLength n) = length a >= n
passwordAllowed a (ContainsSome b) = any (\x -> x `elem` b) a
passwordAllowed a (DoesNotContain b) = not (passwordAllowed a (ContainsSome b))
