...
sums :: Int -> [Int]
sums n = reverse (sumover n n)

sumover :: Int -> Int -> [Int]
sumover n 1= [1]
sumover n i = (i*(i+1) `div` 2) : sumover n (i-1)

reverse :: [Int] -> [Int]
reverse [] = []
reverse arr = reversex arr []
...
sumsOf [] = []
sumsOf [x] = [x]
sumsOf (x:xs) = x: (addon x (sumsOf xs))

addon :: Int -> [Int] -> [Int]
addon x [y] = [x+y]
addon x (y:ys) = (x+y) : (addon x ys)
...
mymaximum bigger initial (x:xs) = let tmp = mymaximum bigger initial xs
                                  in if bigger x tmp then x else tmp
...
maybeMap f [] = []
maybeMap f (x:xs) = let res= f x in 
                    case res of
