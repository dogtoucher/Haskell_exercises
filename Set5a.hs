...
totalPrice (MkShoppingEntry id price number) = price*fromIntegral(number)
...
buyOneMore (MkShoppingEntry id price number) = MkShoppingEntry id price (number+1)
...
data Person = Personal Int String
  deriving Show

-- fred is a person whose name is Fred and age is 90
fred :: Person
fred = Personal 90 "Fred"

-- getName returns the name of the person
getName :: Person -> String
getName (Personal age name) = name 

-- getAge returns the age of the person
getAge :: Person -> Int
getAge (Personal age name) = age

-- setName takes a person and returns a new person with the name changed
setName :: String -> Person -> Person
setName name (Personal age oldname) = (Personal age name)

-- setAge does likewise for age
setAge :: Int -> Person -> Person
setAge age (Personal oldage name) = (Personal age name)
...
study statu = case statu of Freshman -> NthYear 1
                            NthYear 7 -> Graduated
                            Graduated -> Graduated
                            NthYear a -> NthYear (a+1)
...
toNat :: Int -> Maybe Nat
toNat 0 = Just Zero
toNat z = if z>0 then case toNat (z-1) of 
                      Nothing -> Nothing
...
prettyPrint End = ""
prettyPrint (O a) = prettyPrint a ++ "0"
prettyPrint (I a) = prettyPrint a ++ "1"

fromBin :: Bin -> Int
fromBin (O End) = 0
fromBin (O a) = 2*fromBin(a)
fromBin (I End) = 1
fromBin (I a) = 2*fromBin(a) + 1

toBin :: Int -> Bin
toBin 0 = O End
toBin 1 = I End
toBin x = if odd x then (I (toBin (x `div` 2))) else (O (toBin (x `div` 2)))
