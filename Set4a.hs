...
allEqual [] = True
allEqual [x] = True
allEqual (x:y:xs) =  if x==y then True && allEqual (y:xs)
                     else False
...
middle a b c = let ordered=sort[a, b, c] in head (drop 1 ordered)
...
longest :: Ord a => [[a]] -> [a]
longest x = foldl longest' (head x) x 

longest' :: Ord a => [a] -> [a] -> [a]
longest' y x = if (length x > length y) || (length x == length y && x !! 0 < y !! 0) then x else y
------------------------------------------------------------------------------
...
incrementKey key list = map (\(k, v) -> if k == key then (k, v + 1) else (k, v)) list
------------------------------------------------------------------------------
...
winner scores player1 player2 = if (Map.lookup player1 scores) < (Map.lookup player2 scores) then player2 else player1
...
freqs = foldr (\x y -> Map.insertWith (+) x 1 y) Map.empty
...
transfer from to amount bank = if (Map.notMember from bank) || (Map.notMember to bank) || (amount <= 0) || ((Map.!) bank from < amount)
                               then bank
                               else Map.insert from ((Map.!) bank from - amount) (Map.insert to ((Map.!) bank to + amount) bank)
...
swap i j arr =  let temp = arr ! i 
                in arr // [(i, arr ! j), (j, temp)]
...
maxIndex arr = fst (maximumBy (\x y -> compare (snd x) (snd y)) (Data.Array.assocs arr))
